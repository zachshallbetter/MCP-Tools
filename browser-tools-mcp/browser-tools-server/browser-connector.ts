...
import { SecureServer, ServerInfo } from "./secure-server.js";
...
(async () => {
  try {
    ...
    const enableHTTPS = process.env.ENABLE_HTTPS === 'true' || process.env.ENABLE_HTTPS === '1';
    ...
    try {
      serverInfo = await secureServer.start();
    } catch (error) {
      console.error('Failed to start secure server, falling back to HTTP:', error);
      // Fallback to HTTP
      const httpServer = new SecureServer({
        port: PORT,
        host: currentSettings.serverHost,
        enableHTTPS: false,
        app: app
      });
      serverInfo = await httpServer.start();
    }
    ...
    console.log(
      `Aggregator listening on ${protocol}://${currentSettings.serverHost}:${PORT}`
    );
    ...
    console.log(`\nFor local access use: ${protocol}://localhost:${PORT}`);
